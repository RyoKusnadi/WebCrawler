...
func (s *SuiteBase) TestUpsertLink(c *gc.C) {
	...
	original := &graph.Link{
		...
		RetrievedAt: time.Now().Add(-10 * time.Hour).Unix(),
	}
	...
	accessedAt := time.Now().Truncate(time.Second).UTC().Unix()
	existing := &graph.Link{
		ID:          original.ID,
		URL:         "https://example.com",
	...
	sameURL := &graph.Link{
		URL:         existing.URL,
		RetrievedAt: time.Now().Add(-10 * time.Hour).UTC().Unix(),
	}
...
func (s *SuiteBase) TestFindLink(c *gc.C) {
	...
	link := &graph.Link{
		URL:         "https://example.com",
		RetrievedAt: time.Now().Truncate(time.Second).UTC().Unix(),
	}
...
func (s *SuiteBase) TestConcurrentLinkIterators(c *gc.C) {
	...
	for i := 0; i < numIterators; i++ {
		go func(id int) {
			...
			it, err := s.partitionedLinkIterator(c, 0, 1, time.Now().Unix())
...
func (s *SuiteBase) TestLinkIteratorTimeFilter(c *gc.C) {
	...
	linkInsertTimes := make([]int64, len(linkUUIDs))
	for i := 0; i < len(linkUUIDs); i++ {
		link := &graph.Link{URL: fmt.Sprint(i), RetrievedAt: time.Now().Unix()}
		c.Assert(s.g.UpsertLink(link), gc.IsNil)
		linkUUIDs[i] = link.ID
		linkInsertTimes[i] = time.Now().Unix()
	}
...
func (s *SuiteBase) assertIteratedLinkIDsMatch(c *gc.C, updatedBefore int64, exp []uuid.UUID) {
...
func (s *SuiteBase) iteratePartitionedLinks(c *gc.C, numPartitions int) int {
	...
	for partition := 0; partition < numPartitions; partition++ {
		it, err := s.partitionedLinkIterator(c, partition, numPartitions, time.Now().Unix())
		c.Assert(err, gc.IsNil)
		defer func() {
			c.Assert(it.Close(), gc.IsNil)
		}()
...
func (s *SuiteBase) TestUpsertEdge(c *gc.C) {
	...
	c.Assert(edge.UpdatedAt == 0, gc.Equals, false, gc.Commentf("UpdatedAt field not set"))
...
func (s *SuiteBase) TestConcurrentEdgeIterators(c *gc.C) {
	...
	for i := 0; i < numIterators; i++ {
		go func(id int) {
			...
			itTagComment := gc.Commentf("iterator %d", id)
			seen := make(map[string]bool)
			it, err := s.partitionedEdgeIterator(c, 0, 1, time.Now().Unix())
			c.Assert(err, gc.IsNil, itTagComment)
			defer func() {
				c.Assert(it.Close(), gc.IsNil, itTagComment)
			}()
...
func (s *SuiteBase) assertIteratedEdgeIDsMatch(c *gc.C, updatedBefore int64, exp []uuid.UUID) {
...
func (s *SuiteBase) iteratePartitionedEdges(c *gc.C, numPartitions int) int {
	...
	for partition := 0; partition < numPartitions; partition++ {
		...
		linkIt, err := s.partitionedLinkIterator(c, partition, numPartitions, time.Now().Unix())
		c.Assert(err, gc.IsNil)
		for linkIt.Next() {
		...
		it, err := s.partitionedEdgeIterator(c, partition, numPartitions, time.Now().Unix())
		c.Assert(err, gc.IsNil)
		defer func() {
			c.Assert(it.Close(), gc.IsNil)
		}()
...
func (s *SuiteBase) partitionedLinkIterator(c *gc.C, partition, numPartitions int, accessedBefore int64) (graph.LinkIterator, error) {
	from, to := s.partitionRange(c, partition, numPartitions)
	return s.g.Links(from, to, accessedBefore)
}

func (s *SuiteBase) partitionedEdgeIterator(c *gc.C, partition, numPartitions int, updatedBefore int64) (graph.EdgeIterator, error) {
	from, to := s.partitionRange(c, partition, numPartitions)
